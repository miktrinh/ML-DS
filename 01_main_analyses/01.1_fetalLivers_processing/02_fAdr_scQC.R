...
sub_outDir = file.path(outDir,'fAdr')
plotDir = file.path(sub_outDir,'fAdr_')
outPath = file.path(sub_outDir,'fAdr')
metadata = NULL
matchBy = NULL


cleanSrat_fp = ifelse(keepMTCells,paste0(sub_outDir,'/fAdr_clean_withMTCells.RDS'),paste0(sub_outDir,'/fAdr_clean_noMTCells.RDS'))
...
if(file.exists(cleanSrat_fp) & skipIfExists){
  ...
  dataDirs = c(list.dirs('Data/fAdrenals_scRNAseq/',full.names = T,recursive = T))
