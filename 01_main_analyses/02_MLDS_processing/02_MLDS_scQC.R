...
cleanSrat_fp = ifelse(
  keepMTCells,
  paste0(sub_outDir, '/MLDS_clean_withMTCells.RDS'),
  paste0(sub_outDir, '/MLDS_clean_noMTCells.RDS')
)
if (file.exists(cleanSrat_fp) & skipIfExists) {
  ...
  names(dataDirs) = gsub(
    '_GRCh38-2020-A.*$',
    '',
    gsub('^.*_ALeuk_', '', names(dataDirs))
  )
  names(dataDirs) = gsub(
    '_GRCh38-2020-A.*$',
    '',
    gsub('^.*_SB_', '', names(dataDirs))
  )
  names(dataDirs) = gsub(
    '_',
    '.',
    gsub('_GRCh38-2020-A.*$', '', gsub('^.*_MY_', 'MY_', names(dataDirs)))
  )
  ...
  dataDirs = dataDirs[sapply(dataDirs, function(x) {
